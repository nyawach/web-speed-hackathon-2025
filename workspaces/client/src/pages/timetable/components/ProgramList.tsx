...
type ProgramValue = ArrayValues<StandardSchemaV1.InferOutput<typeof schema.getTimetableResponse>>

interface Props {
  channelId: string;
  programList: ProgramValue[];
}

export const ProgramList = ({ channelId, programList }: Props): ReactElement => {
  const [selectedProgram, setSelectedProgram] = useState<ProgramValue | null>(null)

  const onClick = (program: ProgramValue) => {
  ...
  return (
    <>
          ...
          {programList.map((program) => {
            ...
            return (
              <div key={program.id} className="shrink-0 grow-0">
                <Program height={height} program={program} onClick={onClick} />
              </div>
            );
          })}
        </div>
      ...
      {selectedProgram && (
        <ProgramDetailDialog isOpen={!!selectedProgram} program={selectedProgram} onClose={onClose} />
      )}
    </>
  );
};
